...
function Widget() {
    ...
    window.addEventListener('widget-language-change', (event: Event) => {
        const {lang, dir} = (event as CustomEvent).detail;
        console.log('Language changed to:', lang, dir);
    });

    window.addEventListener('widget-theme-change', (event: Event) => {
        const {theme} = (event as CustomEvent).detail;
        setTheme(theme);
    });

    window.addEventListener('widget-debug-change', (event: Event) => {
        const {debug} = (event as CustomEvent).detail;
        console.log('Debug mode:', debug);
    });

    window.addEventListener('widget-user-data', (event: Event) => {
